...
- ~~趨勢圖改版~~ → **已完成**（2026/07/16-17）：三分頁「總資產／台股市值／美股市值(複委託+海外)」，接 `getSnapshots` 真實快照。規格：圖高固定 135px（viewBox 400×160）、區間增減資訊與 20/60/120 快捷在圖上方一列（`.t-head`）、剩餘高度由 auto margin 均分間距、預設近 20 個交易日、手勢連續縮放（手機上下滑／電腦滾輪，Pointer Events）、長按(手機)/點擊拖曳(電腦)對位線＋白底單日資訊框＋右下 ✕ 關閉、每日直向格線標資料點（>60 日簡化）、Y 軸貼合區間、跳過六日、市值 null 略過不補 0、金額配合眼睛遮罩、下拉重新整理（起點在圖表上的手勢不觸發，避免與縮放衝突）。總資產自 7/14、台股/美股自 7/16 起累積
