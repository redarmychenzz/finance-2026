...
- **待重新部署**：本機 `WebApp.gs` 已加入快照市值 3 欄 + `getSnapshots` 端點（2026/07/16），尚未貼到 Apps Script 部署。部署後表頭會自動補成 8 欄
- **趨勢圖改版進行中**：主頁第二分頁要改成三個分頁「總資產／台股市值／美股市值(複委託+海外)」，四種設計範例已出（範例 A 三分 Pill／B 三線疊圖／C 大數字儀表／D 橫滑卡片），等使用者挑選後改 `index.html`。歷史曲線資料來源 = `getSnapshots`（市值欄從部署日才開始累積，舊列為 null 要略過）
- 資產配置頁（alloc）的每日快照 diff 尚有小待辦：後端 `getDashboard` 已回傳 `assetDiff`（總資產較昨收，含銀行變動），前端只存進快取沒有顯示；分配比頁各色塊也還沒有「較昨收」徽章
