...
5. **下拉重新整理**：主頁 / 資產分配比頁 / 股票詳情頁 / 趨勢頁四處都能原地下拉刷新（共用 `attachPullToRefresh`），白色長條律動（等化器）指示。**原生式手感（2026/08/06）**：下拉時內容 1:1 跟手下移（阻尼 `DAMP=0.5`、門檻 `THRESH=70`，`opts.moveEls` 指定位移元素；主頁=home+左側色塊 #bar、趨勢=trendEl、股票詳情=各自捲動容器），指示器在頂部露出的空隙置中；過門檻放手內容停在 70px 陪跑、完成後彈回（`.ptr-drag` 拖曳關過渡／`.ptr-anim` 彈回動畫事後移除）。**分配比頁例外**：色塊有自己的變形動畫，`moveEls:[]` 沿用舊式僅指示器淡入。趨勢頁：捏合/長按進行中 `tChartBusy` 擋下拉。
   **色塊錯位修正（2026/08/06）**：更新套用（`layoutBar`）常發生在內容仍下移陪跑時，`getBoundingClientRect` 會把位移量進卡片座標、烙進色段 top → 彈回後色塊偏低 70px。修法雙保險：(1) `opts.onMove` 回報位移量 `ptrHomeY`，`layoutBar`/`homeBarRects` 量測時扣除；(2) `opts.onSettle` 彈回落地後強制重算一次色塊，任何路徑烙錯座標都會自我修正
...
- **開啟政策 v2（2026/08/06，B 方案）**：
  ...
  - **電腦網頁版重新整理鈕（2026/08/06）**：右上角液態玻璃圓鈕 `#pcRefresh`（40px，同導覽膠囊材質），點擊＝`refreshAll(true)` 全域強制更新、更新中圖示旋轉防連點；**完成時綠勾動畫**（描邊畫出＋按鈕彈跳，1.4 秒後恢復原圖示）。**觸控裝置以 `@media(pointer:coarse)` 隱藏**（手機用下拉）；alloc/curr/rpt 頁開啟時淡出
...
- **Pages 的 HTML 有 10 分鐘 CDN 快取**（`cache-control: max-age=600`）：push 後 1~2 分鐘建置完成，但手機可能再拿到最多 10 分鐘的舊版——測修正時要「完全關閉 PWA 重開」且等快取過期，否則會誤判沒修好（2026/08/06 實際發生過）。可用 `curl -sL https://redarmychenzz.github.io/finance-2026/index.html | grep <新代碼關鍵字>` 確認線上版本
...
- **可讀性(task1，✅完成)**：使用者選定 **A 放大好讀**。App 端 `rptShow` onload 注入 `RPT_INJECT_CSS`(隱藏 #ctrl + body 16.8px/行距 1.95 + `.lede`17.5px + 小字註解/清單 15px + p 段距 + 底部留白 104px 避開浮動導覽膠囊)，三型報告通用、不需改報告端。互動比較預覽 Artifact：`https://claude.ai/code/artifact/6094901b-1498-4ea9-b9c9-b7e9d9fea9c7`(現狀/A/B/C/D)。
- **段落分段**：已在 `daily-report.md`/`gooaye.md`/`haojiao.md` 加「合理分段(一段≤3~4 句約 120 字、長總結拆 2~3 段、能列點就列點)」規範；現有 7/17 日報已手動拆長段(結論先行/一句話/總結)並重新 POST 示範。
